...
function fitted(mimeType: string, bytes: Uint8Array) {
  const result = fitClaudeImage(mimeType, bytes);
  if (!result) throw new Error("expected a fitted image");
...
describe("fitClaudeImage", () => {
  ...
  it("scales a tall phone screenshot to 2000px and keeps its transparency", () => {
    const decoded = PNG.sync.read(fitted("image/png", png(1080, 2424, 128)));
  ...
  it("does not darken colors next to transparent pixels", () => {
    ...
    const decoded = PNG.sync.read(fitted("image/png", PNG.sync.write(image)));
    expect([...decoded.data.subarray(0, 4)]).toEqual([255, 255, 255, 128]);
  });

  it("refuses a PNG too large to decode safely", () => {
  ...
  it("scales a wide JPEG and keeps its EXIF orientation", () => {
    ...
    expect(readImageDimensions(fitted("image/jpeg", data))).toEqual({
      width: 667,
      height: 2000,
    });
  });
